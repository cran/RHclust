VIPnoCPG = function (Simulated = NULL, SNP = NULL, GE = NULL,
  ...
  for (k in lb:ub){
    ...
    while (iter < iter_max){
      ...
      for (g in unique(ge.name$gene)){
        ...
        for (i in 1:nrow(GEcenters)){
          ...
          distance = sqrt(gedistsums) / densum
    ...
    if (nstart > 1){
      for (j in 2:nstart){
        ...
        output.new = VIPnoCPG(Simulated = Simulated, SNP = SNP, GE = GE,
                         SNPname = SNPname, GEname = GEname,
